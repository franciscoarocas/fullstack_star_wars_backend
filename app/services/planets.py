...
from app.utils.order import order_by

from app.constants import NUM_ITEMS_TABLE_PAGE, DEFAULT_DIRECTION
...
class PlanetsService:
  ...
  async def get_planets(self, page, search, sort, direction=DEFAULT_DIRECTION):

    planets_data = await self.__api.get_planets()

    planets_data = search_func(planets_data, search)
    planets_data = sort_func(planets_data, sort)
    planets_data = order_by(planets_data, direction)
